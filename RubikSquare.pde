...
class RubikSquare {
  ...
  void drawPivoting() {
    ...
    if (curmove.frames <= 0) {
      ...
      // STEP 2c: commit the board's state if this is the last pending move
  ...
  void performPivot(PivotMove curmove) {
    if (!curmove.isRow) {
      ...
      for (int i = 0; i < rows; i++) { 
        items[i] = !faces[i][curmove.id];
      }
      items = reverse(items);
      for (int i = 0; i < rows; i++) { 
        faces[i][curmove.id] = items[i];
      }
    } else {
      boolean items[] = new boolean[cols];
      for (int i = 0; i < cols; i++) { 
        items[i] = !faces[curmove.id][i];
      }
      items = reverse(items);
      for (int i = 0; i < cols; i++) { 
        faces[curmove.id][i] = items[i];
      }
    }
  ...
  void drawFace(int i, int j) {
    ...
    // top square
    if (!flipped) { 
      fill(RUBIK_FRONT_COLOR);
    } else { 
      fill(RUBIK_BACK_COLOR);
    }
    rect(0, 0, 1, 1);
    translate(0, 0, -0.001);
  ...
  // return a serialized form of the current board
  void commit() {
    String[] state = new String[rows];

    // serialize the board into something we can restore
    for (int i = 0; i < rows; i++) {
      state[i] = new String("");
      for (int j = 0; j < cols; j++) {
        state[i] += faces[i][j]?"1":"0";
      }
    }

    saveStrings("rubiks_save", state);
  }

  boolean restore() {
    try {
      String[] state = loadStrings("rubiks_save");

      if (state == null || state.length == 0)
        return false;
        
      initialize(state.length, state[0].length);

      // deserialize
      for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
          faces[i][j] = state[i][j] == "1"; 
        }
      }
      ...
      console.warn("Couldn't restore state: ", e);
      return false;
    }
  }
}
